...
func generate_room(dim: Vector2i, danger_level: float) -> RoomData:
	...
	result.scenes[Global.rand_point_in_rect(start_room)] = preload("res://scenes/player.tscn").instantiate()
	result.scenes[Global.rand_point_in_rect(treasure_room)] = preload("res://scenes/rooms/chest.tscn").instantiate()
	
	for i in fight_rooms:
		for j in 3:
			result.scenes[Global.rand_point_in_rect(i)] = preload("res://scenes/enemy/base_enemy.tscn").instantiate()
