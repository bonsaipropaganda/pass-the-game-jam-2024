...
func generate_room(_dim: Vector2i, _danger_level: float) -> RoomData:
	...
	result.scenes[Vector2i(rooms[2].position.x + int(rooms[2].size.x / 2.0),\
		rooms[2].position.y + int(rooms[2].size.y * 3.0 / 4))] = preload("res://scenes/player.tscn").instantiate()
	...
	result.scenes[Vector2i(rooms[0].position.x + int(rooms[0].size.x / 2.0) - 1, rooms[0].position.y - 1)] = preload("res://scenes/rooms/torch.tscn").instantiate()
	result.scenes[Vector2i(rooms[0].position.x + int(rooms[0].size.x / 2.0) + 1, rooms[0].position.y - 1)] = preload("res://scenes/rooms/torch.tscn").instantiate()
	return result
